...
def comparison(actual=None, reference=None, **values):
    actual = actual or score()
    reference = reference or score(label="reference")
    result = dict(rmse=0.01, max_absolute=0.04, cosine=0.99999, all_finite=True,
                  actual_sha256=actual["sha256"], reference_sha256=reference["sha256"],
                  actual_argmax=actual["token"], reference_argmax=reference["token"],
                  reference_margin=reference["margin"], reference="cpu")
...
if __name__ == "__main__":
    tests = [full_case_matrix_and_input_hash, exact_threshold_boundaries,
