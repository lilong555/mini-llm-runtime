...
NCU_EXPORT_FLAGS = ("--csv", "--page", "raw", "--print-units", "base",
                    "--print-kernel-base", "mangled", "--rename-kernels", "0")
...
def analyze_nsys(path, report):
    ...
    try:
        ...
        for row in connection.execute(
                "SELECT start,end,deviceId,contextId,streamId,correlationId,demangledName,mangledName,gridX,gridY,gridZ,"
                "blockX,blockY,blockZ FROM CUPTI_ACTIVITY_KIND_KERNEL ORDER BY start,end"):
            event = dict(row)
            name = names[event.pop("demangledName")]
            symbol = names.get(event.pop("mangledName"))
            ...
            event.update(name=name, mangled_name=symbol, role=role)
            current["events"].append(event)
            events.append(event)
            roles[role] += 1
            grouped[name].append(event["end"] - event["start"])
            if role != "vendor_or_other":
                require(isinstance(symbol, str) and symbol, "NSys 缺少项目 kernel 的原始符号名")
...
def analyze_ncu(path, nsys, report):
    ...
    require(values["ID"] == "0" and values["Device"] == "0" and kernel_role(event["name"]) == "pv"
            and values["Kernel Name"] == event["mangled_name"],
    ...
    return dict(schema_version=1, status="passed", kernel_name=event["name"],
                mangled_kernel_name=values["Kernel Name"], selector=SELECTOR,
...
def validate_bundle(directory):
    ...
    require(export["exit_code"] == 0 and export["executable"] == manifest["tools"]["ncu"]["path"]
            and len(export["arguments"]) == 2 + len(NCU_EXPORT_FLAGS) and export["arguments"][0] == "--import"
            and export["arguments"][2:] == list(NCU_EXPORT_FLAGS),
            "NCU 原始指标导出命令或返回码不符")
    ...
    try:
        if args.schedule:
            print(json.dumps(dict(processes=schedule(), selector=SELECTOR, nsys_flags=NSYS_FLAGS, ncu_flags=NCU_FLAGS,
                                  ncu_export_flags=NCU_EXPORT_FLAGS),
