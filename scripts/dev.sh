...
case "${1:-help}" in
  ...
  micro-benchmark)
    ...
    ;;
  runtime-profile)
    [[ $own_cuda == ON ]] || { echo '用法：bash scripts/dev.sh own-cuda runtime-profile [Profiler 参数]' >&2; exit 1; }
    shift
    exec pwsh -NoProfile -File scripts/Profile-CudaRuntime.ps1 -BinaryDirectory "$build/bin" "$@"
    ;;
  check-http)
    mkdir -p .run
