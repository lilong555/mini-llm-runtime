...
目录移动后可离线重新验收，无需原模型和可执行文件的绝对路径仍有效。归档校验不是可信执行证明。验收先检查必需文件和源码快照；失败写入 `analysis-failure.json`，原始报告与既有汇总保持原字节，成功才原子发布新汇总。历史结果应在临时副本中复验，可用性检查和完整包导出见 [归档可用性与导出](BENCHMARKS.md#归档可用性与导出)。
